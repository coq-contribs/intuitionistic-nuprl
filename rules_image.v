...
(* Lemma hyps_functionality_app {o} :
   forall lib, 
   forall s1 s2 : @CSub o,
   forall H1 H2 : @bhyps o,
      (length s1 = length H1) ->
      (hyps_functionality lib (s1 ++ s2) (H1 ++ H2) <=>
        (hyps_functionality lib s1 H1 # hyps_functionality lib s2 H2)).
Proof.
   intro. intro.  remember (length s1). revert Heqn. induction n; intro; intro; induction H1; introv leneq; inversion leneq.
   simpl. split; intros; dands; repnd; auto.
...
Proof.
  ...
  - clear hyp1.
    autorewrite with core in *. allsimpl.
 ...
 (* we now start proving the sequent *)
  ...
  pose proof (hyp1 (snoc s1a0 (w, a1) ++ s1b) (snoc s2a0 (w, a1) ++ s2b)) as hh.
  clear hyp1.
  dimp hh.
  + (*  hyp functionality *)
     clear hh.
     ...
     repnd; ginv; subst. 
     pose proof (eqh (snoc s2a1 (x, t2) ++ s2b0)) as eqhh.
     autodimp eqhh xx.
     * apply similarity_app.
       eexists; eexists; eexists; eexists; dands; eauto;
       try (allrw length_snoc; omega).
       apply similarity_snoc; simpl.
        eexists; eexists;eexists; eexists. exists w0 p0; dands; eauto.
       lsubst_tac; auto.
    
   
     assert (forall J,
             forall a b : hypothesis,
             !LIn w (dom_csub s1a1) -> 
             !LIn w (free_vars f) -> 
             !LIn w (hyps_free_vars J) -> 
             !LIn x (dom_csub s1a1) -> 
             (LIn (a,b) 
                (combine (substitute_hyps (snoc s1a1 (w, t0))
                          (lsubst_hyps [(x, mk_apply f (mk_var w))] J))
                         (substitute_hyps (snoc s1a1 (x, t1)) J))) ->
                 (cequiv lib (htyp a) (htyp b))).
       { induction J0; introv wns1a1 wnf wnJ xfresh I; simpl in I. destruct I. repndors.
        - destruct a; ginv; simpl.
          eapply cequiv_rw_l_eauto; [apply alpha_eq_sym; apply combine_sub_nest | ].
          unfold lsubst_sub; allsimpl.
          pose proof @lsubst_cequiv_congr.
