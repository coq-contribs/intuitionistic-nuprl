...
Definition per_union2_eq_L {p} lib (ts : cts(p)) T1 T2 t t' : [U] :=
  {x, y : CTerm
   , {eq : per
   , ts T1 T2 eq
     # t ===>(lib) (mkc_inl x)
...
Definition per_union2_eq_R {p} lib (ts : cts(p)) T1 T2 t t' : [U] :=
  {x, y : CTerm
   , {eq : per
   , ts T1 T2 eq
     # t ===>(lib) (mkc_inr x)
